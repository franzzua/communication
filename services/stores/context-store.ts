...
export class ContextStore extends ItemStore<Context, ContextJSON> {
    constructor(private storages: ItemStore<Storage, StorageJSON>,
                private messages: ItemStore<Message, MessageJSON>,
    ...
    protected FromJSON(c: ContextJSON): Context {
        ...
        const result = {
            ...
            Messages: c.MessageURIs.map(mURI => this.messages.getByURI(mURI)),
            Parents: c.ParentsURIs.map(mURI => this.messages.getByURI(mURI))
        };
    ...
    protected async GetURI(item: Context): Promise<string> {
